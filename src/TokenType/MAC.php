...
class MAC extends AbstractTokenType implements TokenTypeInterface
{
    ...
    public function generateResponse()
    {
        ...
        $this->server->getMacStorage()->create($macKey, $this->getParam('accessToken'));
        ...
        if (!is_null($this->getParam('refreshToken'))) {
            $response['refreshToken'] = $this->getParam('refreshToken');
        }
    ...
    public function determineAccessTokenInHeader(Request $request)
    {
        ...
        $calculatedSignature = base64_encode(
                hash_hmac(
                        'sha256', implode("\n", $calculatedSignatureParts), $macKey, true  // raw_output: outputs raw binary data
                )
        );
